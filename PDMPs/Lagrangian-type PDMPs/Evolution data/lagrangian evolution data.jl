...
struct LagrangianEvoData{T, S, N, U}<:EvolutionData
    ...
    #velocity methods
    velocity_ode_parameters::Tuple{MVector{3, Float64}, T, Base.RefValue{Bool}, Base.RefValue{Symbol}}
...
function initialize_evolution_data(pdmp::PDMP{M, F, D, T}, nums::NumericalParameters) where {M<:Lagrangian_Method, F, D, T}
    ...
    #velocity methods
    velocity_ode_parameters = (MVector(0.0, 0.0, 0.0), ini_et, Base.RefValue(false), Base.RefValue(:nil))
