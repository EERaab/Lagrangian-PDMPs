...
function evaluate_flow!(threshold::Float64, pdmp::PDMP, segment::Segment{N}, state::SplitState, evo_data::EvolutionData, 
    ...
    if flow_class.inverse == :None
        ...
        #We solve this separately. For now throw an error
        error("Stationary velocity - as of yet unimplemented.")
    end
    ...
    # We start at t = 0.0 and set up our velocity_heap 
    t = 0.0
    t_prev = 0.0
    J = dyn.component
    u = state.auxiliary[J]
    ...
    for I in eachindex(sign_vector)
        if I ≠ J
            ...
            if sign_vector[I] < 0
                @views segment.reverse_rates[I] = -dot(evo_data.M_adj_rhoJ[I,:], u_tuple)
            end
        end
    end
    ...
    while non_terminal 
        ...
        if isempty(T)
            ...
            u_prev = u
            u = find_zero(G, u, numerics.root_algorithm, integration_parameters)
            non_terminal = false
        else
        ...
        if non_terminal 
            ...
            if ΔΛ ≥ remainder
                #The segment is terminal, but we use a break key-word so no need to flip the bool.
                #non_terminal = false

                #We have a root between t_prev and t -> t overshot:  
                #t_final can be found in [t_prev, t]
                #Correspondingly, u_final can be find in [u_prev, u]
                integration_parameters = (flow_type, param, remainder)
    ...
    for I in eachindex(sign_vector)
        if I ≠ J
            #We check if the fwd rate is positive by checking the sign vector
            if sign_vector[I] > 0
                @views segment.forward_rates[I] = dot(evo_data.M_adj_rhoJ[I,:], u_tuple)
            end
        end
    end
